...
import elevenlabs
import json
from elevenlabs import set_api_key,generate,stream as sr,Voice,VoiceSettings


set_api_key("a9686ac2713a2a45bc98afac869e4280")
...
conversation_history = [
    ...
    {
        "role": "assistant",
        "content": "[TEXT]: hey there! how are ya doing today? \n[EMO]: XJS\n [INT]: XJS\n [SAV]: sav(interacted with new user)"
    }
]
...
def updated_parse_reply(reply):
    ...
    pattern = r'\[(TEXT|GO|WEB|ASK|SAV|ENV|JOB|MSG)\]:\s(.*?)(?=\n?\[|$)'
    matches = re.findall(pattern, reply, re.DOTALL)
    parsed_data = {match[0]: match[1].strip() for match in matches}
    return parsed_data

def process_text(content):
    audio_stream = generate(text=content,  voice=Voice(
        voice_id='w87STgrGJipczC3tgCGc',
...
def process_go(content):
    """Placeholder function for GO."""
    print(f"[GO]: {content}")

def updated_process_web(content):
    """Updated function to handle the new WEB format."""
    if "search(" in content:
        extracted_contentweb = content.split("search(")[1].rstrip(")")
        process_web_search(extracted_contentweb)
    elif "place(" in content:
        extracted_contentweb = content.split("place(")[1].rstrip(")")
        process_web_place(extracted_contentweb)

def process_web_place(extracted_content):
        # set up the request parameters
    websearchparams = {
    'api_key': '004439DBD5F645BEBC276F5433CBA0E6',
    'q': extracted_content
    }
    # make the http GET request to VALUE SERP
    api_result = requests.get('https://api.valueserp.com/locations', websearchparams)

    # print the JSON response from VALUE SERP
    print(json.dumps(api_result.json()))

def process_web_search(extracted_content):
    # set up the request parameters
    websearchparams = {
    'api_key': '004439DBD5F645BEBC276F5433CBA0E6',
    'q': extracted_content
    ...
    # make the http GET request to VALUE SERP
    api_result = requests.get('https://api.valueserp.com/search', websearchparams)
...
def process_sav(content):
    if "send(" in content:
        extracted_contentsav = content.split("search(")[1].rstrip(")")
        process_web_search(extracted_contentsav)
    elif "fetch(" in content:
        extracted_contentsav = content.split("place(")[1].rstrip(")")
        process_web_place(extracted_contentsav)

def longtermsav(content):
    with open('memory.txt', 'a') as file:
        file.write(content)
    print ('done')

def longtermfetch(searchterm):
    with open('memory.txt', 'r') as file:
        content = file.read()
        matches = re.findall(searchterm, content)
        for match in matches:
            print("Found:", match)

def process_ask(content):
    """Placeholder function for ASK."""
    print(f"[ASK]: {content}")

def process_env(content):
    """Placeholder function for ENV."""
    print(f"[ENV]: {content}")

def process_job(content):
    """Placeholder function for JOB."""
    print(f"[JOB]: {content}")

def process_msg(content):
    msgaccount_sid = 'AC96d744eb0d314aa359a3bd7b0162c853'
    msgauth_token = 'b5765d3de4beb497e22e11a791f85e48'
    msgclient = Client(msgaccount_sid, msgauth_token)
...
while True:
    ...
    if "TEXT" in parsed_data:
        process_text(parsed_data["TEXT"])

    # Now, process other commands
    for command, content in parsed_data.items():
        if command != "TEXT":  # We've already processed the TEXT command
            if command == "WEB":
                updated_process_web(content)
            else:
                command_to_function_updated[command](content)
